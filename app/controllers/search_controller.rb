class SearchController < ApplicationController
  ...
  def autocomplete
    query = params[:q].presence || "*" # Check to see if params[:q] is present OR return default value "*" if not present
      ...
      @outfits     = Outfit.search(query, fields: [:name, :occasion, :season], match: :word_start, limit: 5)
      @users       = User.search(query, fields: [:username, :first_name, :last_name], match: :word_start, limit: 5)
  ...
  def search
    ...
    if params[:q].present? && params.has_key?(:q)
      ...
      @pagy_outfits, @outfits         = pagy_searchkick(outfits, fields: [:name, :occasion, :dress_code], match: :word_start, per_page: 10, page_param: :outfits_page, page: params[:page])
