class ShoesController < ApplicationController
  ...
  def create
    ...
    if @shoe.save
      NewProduct.with(product: @shoe).deliver_later(@shoe.brand.favoritors(scope: :brand_follow))
  ...
  def favorite # acts_as_favoritor
    if current_user.favorited? @shoe
      current_user.unfavorite(@shoe, scopes: [:favorite, :product, :shoe])
      redirect_to(shoe_path(@shoe), flash: { warning: "You removed the product #{@shoe.name} from your favorites." })
    else
      current_user.favorite(@shoe, scopes: [:favorite, :product, :shoe])
  ...
  def shoe_params
    params.require(:shoe).permit(:user_id, :brand_id, :name, :description, :retail_price, :type_of, :gender, :materials, :primary_color, :secondary_color, :product_url)
  end
